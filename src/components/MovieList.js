...
const MovieList = ({ type, filteredMovies1, filteredMovies2, movies }) => {

  const watchList = filteredMovies1.filter((movie) => movie.watched === false)
    .map((movie) => (
        ...
        <div className="btn-group btn-group-toggle" data-toggle="buttons">
          <label class="btn active">
            <DeleteSpanner className="badge">
              <DeleteButton movie={movie.id} />
            </DeleteSpanner>
          </label>
          <label class="btn active">
            <Spanner className="badge">
              <WatchButton label="Watched" movie={movie.id} />
            </Spanner>
          </label>
        </div>
      </li>
  ...
  const watchedList = filteredMovies2.filter((movie) => movie.watched === true)
    .map((movie) => (
      <li className="list-group-item d-flex justify-content-between align-items-center">
        <h4>{movie.name}</h4>
        <div className="btn-group btn-group-toggle" data-toggle="buttons">
